...
''' Estudo para o exoplaneta KELT-9 b'''
...
M_planeta_M_jupiter = ((28.4**(-1))*(K_m_s)*((periodo_anos)**(1/3))*((M_estrela_Msol)**(2/3)))#Essa fórmula porque é e=0
print('Massa do planeta em relação a Júpiter = ',M_planeta_M_jupiter)
...
semi_eixo_maior_ua = semi_eixo_maior_m/(1.49*10**11)#divide para transformar em UA
...
R_planeta_R_jupiter = ((profundidade_transito)**(1/2))*(9.94951)*(R_estrela_Rsol)#9.94951 é a razão do raio do Sol sobre o raio de Júpiter
print('Raio do planeta em função de Júpiter = ',R_planeta_R_jupiter)

R_planeta_R_terra = R_planeta_R_jupiter * (69911/6371)#69911/6371 é a razao do raio de Júpiter sobre o raio da Terra
print('Raio do planeta em função da Terra = ',R_planeta_R_terra)

Probabilidade_transito = (R_estrela_m/semi_eixo_maior_m)*100
print('Probabilidade de trânsito porcento = ',Probabilidade_transito)

T_transito = 13*((M_estrela_Msol)**(-1/2))*((semi_eixo_maior_ua)**(1/2))*(R_estrela_Rsol) #tempo de trânsito para e=0 (orbita circular)
